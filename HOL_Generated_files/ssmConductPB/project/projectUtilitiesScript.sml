...
val getPlatoonSergeantCOM_def = Define `
(getPlatoonSergeantCOM ([]:commands option list) = (NONE:commands option))
 /\
(getPlatoonSergeantCOM (( SOME (PlatoonSergeantCOM cmd))::xs) = 
   ( SOME (PlatoonSergeantCOM cmd)))
 /\
(getPlatoonSergeantCOM (_::xs) = getPlatoonSergeantCOM xs)`
...
val getPlatoonSergeantCOMx_def = Define `
(getPlatoonSergeantCOMx ([]:(commands option,principal,'d,'e)Form list) = NONE)
 /\
(getPlatoonSergeantCOMx (((Name PlatoonSergeant) says (prop (SOME (PlatoonSergeantCOM cmd))))::xs) = 
   (SOME (PlatoonSergeantCOM cmd:commands)))
 /\
(getPlatoonSergeantCOMx (_::xs) = getPlatoonSergeantCOMx xs)`
